...
export default function PlayerCard({ attendance, onUpdated }) {
  ...
  async function handleSwipeRemove() {
    const ok = await askConfirm({ title: 'Remove attendance record?', message: 'This will remove the player from this session.', confirmText: 'Remove', tone: 'danger' })
    if (!ok) return
    if (!navigator.onLine) {
